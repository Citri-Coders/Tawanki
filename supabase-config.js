...
// Auto-asegurar datos semilla base en Supabase si están vacíos
(async function ensureBaseCatalogSeeds() {
    try {
        const { data: deptos } = await supabaseClient.from('departamentos').select('id, nombre').limit(10);
        let granadaId = deptos?.find(d => d.nombre === 'Granada')?.id;
        let matagalpaId = deptos?.find(d => d.nombre === 'Matagalpa')?.id;
        let masayaId = deptos?.find(d => d.nombre === 'Masaya')?.id;

        if (!deptos || deptos.length === 0) {
            const { data: insDeptos } = await supabaseClient.from('departamentos').insert([
                { nombre: 'Granada', region: 'Pacífico' },
                { nombre: 'Masaya', region: 'Pacífico' },
                { nombre: 'León', region: 'Pacífico' },
                { nombre: 'Rivas', region: 'Pacífico' },
                { nombre: 'Matagalpa', region: 'Centro-Norte' },
                { nombre: 'Estelí', region: 'Centro-Norte' },
                { nombre: 'Managua', region: 'Pacífico' }
            ]).select();
            if (insDeptos) {
                granadaId = insDeptos[0]?.id;
                matagalpaId = insDeptos[4]?.id || granadaId;
                masayaId = insDeptos[1]?.id || granadaId;
            }
        }

        const { data: rubros } = await supabaseClient.from('rubros').select('id, nombre').limit(10);
        let cafeRubroId = rubros?.find(r => r.nombre && (r.nombre.includes('Gastronomía') || r.nombre.includes('Café')))?.id;
        let barroRubroId = rubros?.find(r => r.nombre && r.nombre.includes('Artesanías'))?.id;
        let textilRubroId = rubros?.find(r => r.nombre && r.nombre.includes('Textiles'))?.id;

        if (!rubros || rubros.length === 0) {
            const { data: insRubros } = await supabaseClient.from('rubros').insert([
                { nombre: 'Gastronomía & Café', descripcion: 'Café especial y alimentos', icono: 'coffee' },
                { nombre: 'Artesanías & Barro', descripcion: 'Cerámica y souvenirs', icono: 'palette' },
                { nombre: 'Madera & Muebles', descripcion: 'Muebles rústicos y madera', icono: 'carpenter' },
                { nombre: 'Textiles & Hamacas', descripcion: 'Hamacas y textiles', icono: 'chair' },
                { nombre: 'Frutas & Hortalizas', descripcion: 'Frutas y vegetales', icono: 'nutrition' }
            ]).select();
            if (insRubros) {
                cafeRubroId = insRubros[0]?.id;
                barroRubroId = insRubros[1]?.id;
                textilRubroId = insRubros[3]?.id;
            }
        }

        // Si la tabla de productos está vacía, crear productos auténticos
        const { data: existingProds } = await supabaseClient.from('productos').select('id').limit(1);
        if (!existingProds || existingProds.length === 0) {
            const { data: prods } = await supabaseClient.from('productores').select('id').limit(1);
            let sampleProdId = prods?.[0]?.id;
            if (!sampleProdId) {
                const { data: newSampleProd } = await supabaseClient.from('productores').insert({
                    nombre_cooperativa_o_taller: 'Cooperativa Las Brisas / San Juan de Oriente',
                    departamento_id: granadaId || 1,
                    rubro_id: cafeRubroId || 1,
                    telefono_whatsapp: '+505 8888-7777',
                    descripcion: 'Productores artesanales de café de altura, cacao criollo y cerámica tradicional.',
                    verificado_intur: true
                }).select().maybeSingle();
                sampleProdId = newSampleProd?.id;
            }

            if (sampleProdId) {
                await supabaseClient.from('productos').insert([
                    {
                        productor_id: sampleProdId,
                        rubro_id: cafeRubroId || 1,
                        nombre: 'Café Orgánico de Altura (Caturra Lavado)',
                        descripcion: 'Café de estricta altura 1,200 msnm con notas a chocolate y miel. Tueste medio para hotelería y restaurantes.',
                        precio_unitario: 180,
                        unidad_medida: 'lb',
                        badge_distintivo: '500 lbs/mes',
                        imagen_url: 'https://images.unsplash.com/photo-1559056199-641a0ac8b55e?w=500&auto=format&fit=crop&q=80',
                        disponible: true
                    },
                    {
                        productor_id: sampleProdId,
                        rubro_id: barroRubroId || 2,
                        nombre: 'Juego de Tazas y Vasijas en Barro Bruñido',
                        descripcion: 'Piezas artesanales de San Juan de Oriente moldeadas a mano, acabados en engobe natural apto para servicio de alimentos.',
                        precio_unitario: 250,
                        unidad_medida: 'juego (6 pcs)',
                        badge_distintivo: '50 juegos/mes',
                        imagen_url: 'https://images.unsplash.com/photo-1578749556568-bc2c40e68b61?w=500&auto=format&fit=crop&q=80',
                        disponible: true
                    },
                    {
                        productor_id: sampleProdId,
                        rubro_id: textilRubroId || 4,
                        nombre: 'Hamaca Matrimonial Tejido Masaya con Flecos',
                        descripcion: 'Hamacas de hilo 100% algodón tejido fino con maderas de guásimo laqueadas. Ideal para terrazas de hoteles boutique.',
                        precio_unitario: 1450,
                        unidad_medida: 'unidad',
                        badge_distintivo: '20 unid/mes',
                        imagen_url: 'https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?w=500&auto=format&fit=crop&q=80',
                        disponible: true
                    },
                    {
                        productor_id: sampleProdId,
                        rubro_id: cafeRubroId || 1,
                        nombre: 'Cacao Criollo Fermentado en Grano y Nibs',
                        descripcion: 'Cacao fino de aroma certificado 70% pureza, secado al sol en tarimas de madera para repostería y chocolatería artesanal.',
                        precio_unitario: 120,
                        unidad_medida: 'kg',
                        badge_distintivo: '300 kg/mes',
                        imagen_url: 'https://images.unsplash.com/photo-1548848221-0c2e497ed557?w=500&auto=format&fit=crop&q=80',
                        disponible: true
                    }
                ]);
            }
        ...
        console.warn("Aviso en ensureBaseCatalogSeeds:", e);
    }
})();
